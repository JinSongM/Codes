...
def metdig_plot_s(infile_list, outfile, ini_config, stafile, Bool='F'):
    ...
    try:
        ...
        obs_time_dc = '{0:%Y}年{0:%m}月{0:%H}时平均值'.format(datetime.strptime(file_time, '%Y%m%d%H'))
        ...
        try:
            ...
            if ini_config[0].__contains__('PC'):
                if ini_config[0].__contains__('风速') or ini_config[0].__contains__('风向'):
                    staGrd, extend = statistical_Inspect_sta(infile_list, stafile, 'PC-wind')
                else:
...
def metdig_plot_c(infile_list, outfile, ini_config, stafile, Bool='F'):
    ...
    try:
        ...
        if outfile.__contains__('wind'):
            file_time = os.path.split(fst_file)[1].split('.')[0].split('_')[-1]
        else:
            file_time = os.path.split(fst_file)[1].split('.')[0]
        Fst_time = int(os.path.split(fst_file)[1].split('.')[1])
        png_name = os.path.basename(outfile)
        obs_time_dc = '{0:%Y}年{0:%m}月{0:%H}时平均值'.format(datetime.strptime(file_time, '%Y%m%d%H'))
        forcast_info = obs_time_dc + '\n' + '预报时效：{}小时'.format(Fst_time)
        ...
        try:
            ...
            if ini_config[0].__contains__('PC'):
                if ini_config[0].__contains__('风速') or ini_config[0].__contains__('风向'):
                    arrayGrd, extend = statistical_Inspect_grd(infile_list, 'PC-wind')
                else:
...
def statistical_Inspect_sta(infile_list, stafile, index):
    ...
    if index == 'PC':
        ...
        extend = 'neither'
    if index == 'PC-wind':
        array_PC = np.nanmean(np.array([np.where(np.abs(i)==1., 100,  0) for i in sta_list]), axis=0)
        array_PC[array_PC < 50.] = np.nan
        sta.data0 = array_PC
        extend = 'neither'
    return sta, extend
...
def statistical_Inspect_grd(infile_list, index):
    ...
    if index == 'PC':
        ...
        extend = 'neither'
    if index == 'PC-wind':
        array = np.nanmean(np.array([np.where(np.abs(i)==1., 100,  0) for i in array_list]), axis=0)
        grd.data = [[[[array]]]]
        extend = 'neither'
...
def gain_sta(stafile, model_file):
    ...
    if model_file.__contains__('wind'):
        ...
        if label == 'ACD':
            data_change = np.where(
                (DF2_merge['acd'].astype(np.float) == 9999.), np.nan,
                DF2_merge['acs'].astype(np.float))
        elif label == 'ACS':
            data_change = np.where(
                (DF2_merge['acs'].astype(np.float) == 9999.), np.nan,
                DF2_merge['acs'].astype(np.float))
        else:
            data_change = np.where(
                (DF2_merge['fst'].astype(np.float) == 9999.) | (DF2_merge['obs'].astype(np.float) == 9999.), np.nan,
                DF2_merge['fst'].astype(np.float) - DF2_merge['obs'].astype(np.float))
    ...
    if var == 'wind':
        if lev.__contains__('station'):
            ...
            if label == 'ACS':
                ...
                lab = 'ws_me'
            for report_time in RT_list:
                infile = windfile_format.format(lab=lab, var=var, model=model, lev=lev_filename, RT=report_time, cst=cst)
                if os.path.exists(infile):
                    infile_list.append(infile)
    else:
        if lev.__contains__('station'):
