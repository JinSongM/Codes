...
class PictureDrawTask:
    ...
    def pic_draw_7(self):
        ...
        sum_vb, sum_ve = 0, 0
        for v_b in rate_begin.values():
            if datetime.strptime(list(rate_begin.keys())[0],'%Y%m%d') >= datetime(self.date_time.year,4,21):
                sum_vb += v_b
                if sum_vb >= len(self.sta_obj_list) * 0.6:
                    region_begin_rainy_day = rate_begin.keys()
        for v_e in end_begin.values():
            if datetime.strptime(list(rate_begin.keys())[0],'%Y%m%d') >= datetime(self.date_time.year,9,21):
                sum_ve += v_e
                if sum_ve >= len(self.sta_obj_list) * 0.6:
                    region_end_rainy_day = end_begin.keys()
        print(sum_vb, sum_ve)
        print(region_begin_rainy_day, region_end_rainy_day)
        # 计算雨季持续时间和降水强度
        if region_begin_rainy_day:
            pre_start = min(list(region_begin_rainy_day))
        else:
            pre_start = '未检测出'
        if region_end_rainy_day:
            pre_end = min(list(region_end_rainy_day))
        else:
            pre_end = '未检测出'
        if region_begin_rainy_day and region_end_rainy_day:
            pre_lenth = (pd.to_datetime(pre_end) - pd.to_datetime(pre_start)).days
            pre_strength, pre_strength_class = self.calculate_pre_strength(min(list(region_begin_rainy_day)),
                                                                           min(list(region_end_rainy_day)))
        else:
            pre_lenth = '未检测出'
