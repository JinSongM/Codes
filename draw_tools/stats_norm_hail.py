...
class loadData:
    ...
    def __init__(self):
        # 冰雹数据
        self.hail0_file = r'/data/PRODUCT/rain_channel_sta/0/merge3/merge_rain0.csv'
        self.hail1_file = r'/data/PRODUCT/hail_channel_sta/0/merge3/merge_hail0.csv'
        self.hail2_file = r'/data/PRODUCT/hail_channel_sta/1/merge3/merge_hail1.csv'
        try:
            self.hail_df0 = pd.read_csv(self.hail0_file)
            self.hail_df1 = pd.read_csv(self.hail1_file)
            self.hail_df2 = pd.read_csv(self.hail2_file)

            self.hail0_df = self.hail_df0.dropna(axis=0, how='any')
            self.hail1_df = pd.concat([self.hail_df1, self.hail_df2], axis=0)
            self.hail1_df = self.hail1_df.dropna(axis=0, how='any')

            # # 标签检验
            # labelDf0 = self.hail0_df[channel_list].copy().sample(10000)
            # labelDf0['label'] = 0
            # labelDf1 = self.hail1_df[channel_list].copy().sample(frac=0.3)
            # labelDf1['label'] = 1
...
class drawMembership(loadData):
    ...
    def eigenvalue(self, channels):
        filepath = os.path.join(outpathdir, 'eigenvalue.csv')
        if not os.path.exists(outpathdir):
            os.makedirs(outpathdir)

        if os.path.exists(filepath):
            eigenvalue_dict = pd.read_csv(filepath)[channel_list].to_dict('list')
        else:
            eigenvalue_dict = {}
            for channel in channels:
                try:
                    xdata_0 = np.sort(self.hail0_df.get(channel).to_list())
                    mean_0, std_0 = norm.fit(xdata_0)
                    xdata_1 = np.sort(self.hail1_df.get(channel).to_list())
                    mean_1, std_1 = norm.fit(xdata_1)
                    eigenvalue_dict[channel] = [mean_0, std_0, mean_1, std_1]
                except:
                    return None
            pd.DataFrame(eigenvalue_dict).to_csv(filepath)
        ...
        if os.path.exists(filepath):
            ...
            try:
                ...
                for CN, VA in self.eigenvalue_dict.items():
                    ...
                    area = np.trapz(np.min([norm.pdf(xdata, VA[2], VA[3]), norm.pdf(xdata, VA[0], VA[1])], axis=0), xdata)
                    area_dict[CN] = [area]
    ...
    def membership_weight(self, channel, x):
        try:
            ...
            membership = norm.pdf(x, value[2], value[3]) / (
                        norm.pdf(x, value[0], value[1]) + norm.pdf(x, value[2], value[3]))
    ...
    def draw_norm(self, pic_name, ytitle, ytitle2):
        ...
        for channel, value in self.eigenvalue_dict.items():
            try:
                ...
                y1, y2 = norm.pdf(x, value[0], value[1]), norm.pdf(x, value[2], value[3])
                y = [min(x, y) for x, y in zip(y1, y2)]
                ax.fill_between(x, y, color='blue', alpha=0.2)

                ax.plot(x, y1, 'g--', lw=2, alpha=0.6, label='无冰雹')
                ax.plot(x, y2, 'y-', lw=2, alpha=0.6, label='小冰雹')
                plt.legend(frameon=False, loc=1, prop={'family': 'simsun'})
...
class MonitorConvection:
    ...
    def MonitorConvection(self):
        ...
        try:
            ...
            fy4b_channel_dict = {
                'BT13': cv2.GaussianBlur(fy4b_obj.tb_13, (5, 5), 0, 0),
                'BTD14_13': cv2.GaussianBlur(fy4b_obj.tb_14 - fy4b_obj.tb_13, (5, 5), 0, 0),
                'BTD09_13': cv2.GaussianBlur(fy4b_obj.tb_09 - fy4b_obj.tb_13, (5, 5), 0, 0),
                'BTD09_10': cv2.GaussianBlur(fy4b_obj.tb_09 - fy4b_obj.tb_10, (5, 5), 0, 0),
            }
            print('完成通道数据字典准备')
    ...
    def draw_haillabel(self, LabelGrd, background):
        try:
            ...
            map.set_xticks(np.arange(west_lon, east_lon + res, 2), crs=ccrs.PlateCarree())
            map.set_yticks(np.arange(south_lat, north_lat + res, 2), crs=ccrs.PlateCarree())
            # map.set_extent([west_lon, east_lon, south_lat, north_lat], crs=ccrs.PlateCarree())
            map.set_extent([116, 124, 30, 36], crs=ccrs.PlateCarree())
            ...
            plt.colorbar(BackGroundShow, aspect=45, pad=0.05, shrink=0.8, orientation='horizontal')
            ...
            # xr_data_array = cv2.erode(xr_data_array, kernel, iterations=1)
            xr_data_array = cv2.GaussianBlur(xr_data_array.astype(np.uint8), (3, 3), 0, 0)
            xr_data_array = np.ma.masked_where(xr_data_array != 1, xr_data_array)
            plt.pcolormesh(LON, LAT, xr_data_array, cmap=colors.ListedColormap(['#FF99CC']))

            # 绘制散点图
            filepath = r'D:\Wiztek_Python\cartopy_draw\plot\data\20231128下载数据\bighailpro\data\20230919\202309192000.000'
            ...
            plt.scatter(lon_list, lat_list, marker='^', s=30, c='r')

            map.add_geometries(shp.geometries(), crs=ccrs.PlateCarree(), edgecolor='b', linewidths=0.6,
                               facecolor='none', alpha=0.6)
            filepath = os.path.join(outpathdir, 'hail_' + self.date_time.strftime('%Y%m%d%H%M') + '.png')
            plt.savefig(fname=filepath, bbox_inches='tight')
            print('绘图成功：{}'.format(os.path.basename(filepath)))
        except Exception as e:
            print(e)
...
if __name__ == '__main__':
    ...
    drawNormClass = drawMembership(channel_list)
    # drawNormClass.draw_norm('冰雹-FY4B卫星通道概率密度-隶属度分布', '概率密度', '隶属度')
    # TB_dict = {
    #     'BT13': 251.5700073,
