# -- coding: utf-8 --
# @Time : 2023/11/20 19:45
# @Author : 马劲松
# @Email : mjs1263153117@163.com
# @File : stats_norm_wind_kde.py
# @Software: PyCharm
...
class loadData:
    ...
    def __init__(self):
        # 强降水数据
        self.wind0_file = r'/data/PRODUCT/wind_channel_sta/0/merge3/merge_wind0.csv'
        self.wind1_file = r'/data/PRODUCT/wind_channel_sta/8/merge3/merge_wind8.csv'
        self.wind2_file = r'/data/PRODUCT/wind_channel_sta/10/merge3/merge_wind10.csv'
        self.wind3_file = r'/data/PRODUCT/wind_channel_sta/12/merge3/merge_wind12.csv'
        try:
            self.wind_df0 = pd.read_csv(self.wind0_file)
            self.wind_df1 = pd.read_csv(self.wind1_file)
            self.wind_df2 = pd.read_csv(self.wind2_file)
            self.wind_df3 = pd.read_csv(self.wind3_file)

            self.wind0_df = self.wind_df0.dropna(axis=0, how='any')
            self.wind1_df = pd.concat([self.wind_df1, self.wind_df2, self.wind_df3], axis=0)
            self.wind1_df = self.wind1_df.dropna(axis=0, how='any')

            # 标签检验
            labelDf0 = self.wind0_df[channel_list].copy().sample(2000)
            labelDf0['label'] = 0
            labelDf1 = self.wind1_df[channel_list].copy().sample(2000)
            labelDf1['label'] = 1
...
class drawMembership(loadData):
    ...
    def eigenvalue(self, channels):
        ...
        for channel in channels:
            try:
                xdata_0 = np.sort(self.wind0_df.get(channel).to_list())
                kde0 = gaussian_kde(xdata_0)
                xdata_1 = np.sort(self.wind1_df.get(channel).to_list())
...
class MonitorConvection:
    ...
    def draw_windlabel(self, LabelGrd, background):
        try:
    ...
    def mainProcess(self, threshold):
        ...
        windLabelGrd = self.creat_M4_grd(lon_grd, lat_grd, label, 'windlabel', gtime)
        self.draw_windlabel(windLabelGrd, fy4b_channel_dict.get('BT13'))

if __name__=='__main__':
    threshold = 0.9
    outpathdir = r'D:\Wiztek_Python\cartopy_draw\plot\test\wind'
