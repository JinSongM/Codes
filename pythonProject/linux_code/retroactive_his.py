...
def read_typeTP(obs, obs6, Ppt1, Ppt2, Ppt2_sta, obs_t):
    ...
    obs_data_filter = obs_data[obs_data['data0'] < 9999.0]
    ...
    df_merge = pd.merge(obs_data_filter, obs6_data, how='inner', on='id').merge(c1d_sta_data, how='inner', on='id') \
...
def proc(start_day, end_day, cst, outPATH, rp_T, label):
    out_file = outPATH.format(d_time=start_day, fst=cst, label=label, HR=rp_T)
    begin_day = start_day
    merge_df = pd.DataFrame()
    while begin_day <= end_day:
        obs_t = begin_day + timedelta(hours=cst)
        land_obs6_file = land_obs6.format(d_time=obs_t)
        land_obs_file = land_obs.format(d_time=obs_t)
        c3e_Ppt1_file = c3e_Ppt1.format(d_time=begin_day, fst=cst)
        c1d_Ppt2_file = c1d_Ppt2.format(d_time=begin_day, fst=cst)
        c1d_Ppt2_sta_file = c1d_Ppt2_sta.format(d_time=begin_day, fst=cst)
