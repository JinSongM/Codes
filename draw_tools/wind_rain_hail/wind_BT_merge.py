# -- coding: utf-8 --
# @Time : 2023/11/29 14:03
# @Author : 马劲松
# @Email : mjs1263153117@163.com
# @File : wind_BT_merge.py
# @Software: PyCharm
import os, glob
import sys, cv2
import numpy as np
...
def get_wind_file(d_date: datetime):
    ...
    try:
        ...
        wind_sta_8 = wind_sta[(wind_sta.data0 >= 17.2) & (wind_sta.data0 < 24.5)]
        wind_sta_10 = wind_sta[(wind_sta.data0 >= 24.5) & (wind_sta.data0 < 32.7)]
        wind_sta_12 = wind_sta[wind_sta.data0 >= 32.7]
        wind_dict['8'] = wind_sta_8
        wind_dict['10'] = wind_sta_10
        wind_dict['12'] = wind_sta_12
        return wind_dict
    except:
        return None

def get_channel_sta(time_list):
    dict_BT_wind8 = {key:[] for key in label_list}
    dict_BT_wind10 = {key:[] for key in label_list}
    dict_BT_wind12 = {key:[] for key in label_list}
    ...
    for time_obs in time_list:
        try:
            ...
            for key, value in grd_channels.items():
                if not wind_dict.get('8').empty:
                    sta_rain20 = meb.interp_gs_nearest(value, wind_dict.get('8'))[['time', 'lon', 'lat', key]]
                    dict_BT_wind8.get(key).append(sta_rain20)
                if not wind_dict.get('10').empty:
                    sta_rain50 = meb.interp_gs_nearest(value, wind_dict.get('10'))[['time', 'lon', 'lat', key]]
                    dict_BT_wind10.get(key).append(sta_rain50)
                if not wind_dict.get('12').empty:
                    sta_rain80 = meb.interp_gs_nearest(value, wind_dict.get('12'))[['time', 'lon', 'lat', key]]
                    dict_BT_wind12.get(key).append(sta_rain80)
            print('##' * 40)
        except Exception as e:
            print(e)
    BT_list_wind8, BT_list_wind10, BT_list_wind12 = list(dict_BT_wind8.values()), list(dict_BT_wind10.values()), list(dict_BT_wind12.values())

    print('保存特征值统计文件')
    write_channel_sta_to_csv(BT_list_wind8, 8)
    write_channel_sta_to_csv(BT_list_wind10, 10)
    write_channel_sta_to_csv(BT_list_wind12, 12)
