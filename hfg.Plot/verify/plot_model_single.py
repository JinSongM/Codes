...
def draw_hgt_uv_wsp_sg(u, v, wsp, map_extent, regrid_shape, Filter_min, Filter_max, **kwargs):
    ...
    obj = horizontal_compose(title=title, description=forcast_info, png_name=png_name, add_tag=False,
    ...
    gc.collect()
    uv_barbs(obj.ax, u, v, regrid_shape=r_Shape, sizes=dict(emptybarb=0), length=5)
    obj.save()

def draw_hgt_uv_wsp_sg_cldas(u_c, v_c, wsp, map_extent, disc, regrid_shape, Filter_min, Filter_max, **kwargs):
    data_name = str(u_c['member'].values[0])
    title = '[{}]风速'.format(
        data_name.upper())
    forcast_info = '实况时间：' + disc[-18:-14] + '年' + disc[-14:-12] + '月' + disc[-12:-10] + '日' + disc[-10:-8] + '时'
    ...
    wsp_pcolormesh(obj.ax, wsp, levels=[2, 4, 6, 8, 10, 12, 14], colorbar_kwargs={'pad': 0.05, 'extend': 'both'}, cmap='ncl/wind_17lev', alpha=1)
    gc.collect()
    uv_barbs(obj.ax, u_c, v_c, regrid_shape=r_Shape, sizes=dict(emptybarb=0), length=5)
    obj.save()
...
def horizontal_pallete_test(ax=None, figsize=(16, 9), crs=ccrs.PlateCarree(), map_extent=(60, 145, 15, 55),
    ...
    # 增加坐标
    if add_ticks:
        if map_extent[1] - map_extent[0] >= 10:
            xstep = 10
        else:
            xstep = 2
        if map_extent[3] - map_extent[2] >= 10:
            ystep = 10
        else:
            ystep = 2

        if (isinstance(add_ticks, bool)):
            utl_plotmap.add_ticks(ax, xticks=np.arange(map_extent[0], map_extent[1] + 1, xstep),
                                  yticks=np.arange(map_extent[2], map_extent[3] + 1, ystep))
        else:
            utl_plotmap.add_ticks(ax, xticks=np.arange(map_extent[0], map_extent[1] + 1, xstep),
                                  yticks=np.arange(map_extent[2], map_extent[3] + 1, ystep))
...
class PlotHgtWindProd_sg:
    ...
    def plot_hgt_wind_speed_sg(self, region_name, region, file_path, Filter_min=-9999.0, Filter_max=9999.0, is_overwrite=False):
        """
        高度场和风场
        :param lev:
        :param report_time:
        ...
        if self.model_name == 'CLDAS':
            draw_hgt_uv_wsp_sg_cldas(self.u, self.v, self.wind_speed, map_extent=region,
                                     regrid_shape = [region_name],
                                     output_dir=os.path.dirname(file_path), disc=os.path.basename(file_path),
                                     png_name=os.path.basename(file_path), is_overwrite=is_overwrite, add_city=False,
                                     Filter_min=Filter_min, Filter_max=Filter_max)
        else:
            draw_hgt_uv_wsp_sg(self.u, self.v, self.wind_speed, map_extent=region, output_dir=os.path.dirname(file_path),
                               png_name=os.path.basename(file_path), is_overwrite=is_overwrite, add_city=False,
                               regrid_shape = [region_name],
                               Filter_min=Filter_min, Filter_max=Filter_max)