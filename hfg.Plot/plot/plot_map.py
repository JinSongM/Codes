...
class DrawImages(object):
    def __init__(self, shp_idx=None, axes=None, extent=None, x_scale=10, y_scale=10, central_longitude=160, dpi=400,
                 region=None):
    ...
    def add_terrain(self):
        backmap = cimgt.Stamen('terrain-background')
        # backmap=cimgt.OSM()
        # backmap=cimgt.Stamen()

        # backmap = cimgt.QuadtreeTiles()
        self.ax.add_image(backmap, 6)
    ...
    def _add_china_shp(self, ax):
        ...
        continents = ShapelyFeature(
            Reader('{}/new_shp/world_coastline_new_dissolve.shp'.format(self.shp_folder)).geometries(),
            ccrs.PlateCarree())
        ax.add_feature(continents, facecolor='None', edgecolor="#111111", linewidth=0.4, zorder=1)

        province = ShapelyFeature(
            Reader('{}/new_shp/BOUL_S.shp'.format(self.shp_folder)).geometries(),
            ccrs.PlateCarree())
        ax.add_feature(province, facecolor='None', edgecolor="#111111", linewidth=0.4, zorder=1)

        jdx = ShapelyFeature(Reader('{}/new_shp/BOUL_JDX.shp'.format(self.shp_folder)).geometries(),
                             ccrs.PlateCarree())
        ax.add_feature(jdx, facecolor='None', edgecolor="#111111", linewidth=3.2, zorder=1)
    ...
    def _draw_world_map(self, resolution="110m", ocean_color="#aadaff", land_color="#ffffff", lakes_color="#aadaff",
        ...
        if add_world_countries:
            all_countries = ShapelyFeature(
                Reader(
                    '{}/{}/world_country_new_dissolve.shp'.format(self.shp_folder, "shp_glb/new_shp"), ).geometries(),
                ccrs.PlateCarree())
            self.ax.add_feature(all_countries, facecolor='None', edgecolor="black", linewidth=0.4, zorder=1)
            ...
            dalu = ShapelyFeature(
                Reader('{}/{}/dalu110.shp'.format(self.shp_folder, "shp_glb/new_shp"), ).geometries(),
                ccrs.PlateCarree())
            self.ax.add_feature(dalu, facecolor='None', edgecolor="black", linewidth=0.5, zorder=1)
    ...
    def set_title(self, title=None, y=0.75, family="helvetica"):
        ...
        if title is not None:
            self.ax.set_title(title, zorder=0, fontdict=font_family, y=y)
