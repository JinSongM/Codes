...
__all__ = ["save_array", "load_array", "get_parent_dir", "mkdir", "cartopy_subplot", "read_m3_file",
           ...
           "low_pass_filter", "bw_filter", "notch_filter", "creat_M4_grd", 'save_uv_to_m11', 'save_nc', 'convert_time']

def concate_array(dataarray1, dataarray2):
    """
    params:
        dataarray1: 待拼接的数组
        dataarray2: 待拼接的数组
    return:
        ...
        上下拼接后的数组: axis=0
    """
    concate_array = np.concatenate([dataarray1, dataarray2], axis=0)
...
#时间规整
def convert_time(date_time: datetime, interval):
    """
    params:
        date_time: datetime时间
        interval: 取整分钟
    return:
        取整后的datetime
    """
    time_cov = datetime.fromtimestamp(round(date_time.timestamp() / (interval * 60)) * (interval * 60))
    return  time_cov
