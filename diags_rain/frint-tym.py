...
##################################################################
fcstroot='/g1/u/nwp_vfy/houtai/MUVOS/forecast/REGION/TYM/pgbf'
outputroot='/g3/vfy_xp/diags_rain/CMA-TYM/frint'
###################################################################
...
########### read obs ####################################################
				...
				if os.access(obsfile,os.F_OK) and os.path.getsize(obsfile):
					...
					lat=np.arange(-15,60.06+0.09,0.09)
					lon=np.arange(40,180.04+0.09,0.09)
					for line in infile:
						words = line.split()
						...
						tx=int((obslat+15)/resolution)
						ty=int((obslon-40)/resolution)
