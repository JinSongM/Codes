...
class MergeStationData:
    ...
    def _read_obs_data(self):
        ...
        for sta_id in ids:
            ...
            tmp_date = obs_end_time.replace(month=8, day=20)
            while tmp_date <= obs_end_time:
                station_result = {}
                time_str = tmp_date.strftime('%y%m%d')
                args.append([time_str,input_path,sta_id,station_result])
                tmp_date += timedelta(days=1)

        pool = mp.Pool(processes=12)
        res = pool.starmap_async(self._read_obs_data_mp, args)
        result = res.get()
        pool.close()
        pool.join()
    ...
    def _read_obs_data_mp(self,time_str,input_path,sta_id,station_result):
        file_name = time_str + self.start_h + ".000"
        ...
        if os.path.exists(file_path):
            try:
                ...
                if sta_id in sta_data_dict.keys():
                    result =  time_str + '\t' + sta_data_dict.get(sta_id) + '\n'
                else:
                    result =  time_str + "\t" + '0.0' + '\n'
...
def prepare_station_obs_and_fst_data(date_time: datetime, start_h: str):
    ...
    sta_08_obj = sta_info.StationInfo_08(sta_obj_dict, date_time, start_h)
    return sta_08_obj
