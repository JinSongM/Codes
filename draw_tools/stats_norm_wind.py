...
class loadData:
    ...
    def __init__(self):
        # 雷暴大风数据
        self.rain0_file = r'/data/PRODUCT/wind_channel_sta/0/merge3/merge_wind0.csv'
        self.rain1_file = r'/data/PRODUCT/wind_channel_sta/8/merge3/merge_wind8.csv'
        self.rain2_file = r'/data/PRODUCT/wind_channel_sta/10/merge3/merge_wind10.csv'
        self.rain3_file = r'/data/PRODUCT/wind_channel_sta/12/merge3/merge_wind12.csv'
        try:
    ...
    def draw_norm(self, pic_name, ytitle, ytitle2):
        ...
        for channel, value in self.eigenvalue_dict.items():
            try:
                ...
                ax.plot(x, y1, 'g--', lw=2, alpha=0.6, label='0~8')
                ax.plot(x, y2, 'y-', lw=2, alpha=0.6, label='8~')
...
class MonitorConvection:
    ...
    def MonitorRadar(self):
        ...
        try:
            ...
            meb.write_griddata_to_micaps4(radar_grd_inter, r'D:\mnt\radar_data\SWAN\MCR\2023\20230919\1.000')
    ...
    def draw_rainlabel(self, LabelGrd, background):
        try:
            ...
            plt.title('雷暴大风', loc='left', fontsize=20, fontdict={'family': 'simhei'})
            ...
            map.set_extent([110, 118, 30, 36], crs=ccrs.PlateCarree())
            ...
            plt.pcolormesh(LON, LAT, xr_data_array, cmap=colors.ListedColormap(['#BEF3D9']))

            # 绘制散点图
            sta = meb.read_stadata_from_micaps1_2_8(r'D:\Wiztek_Python\cartopy_draw\plot\data\20231128下载数据\gust\data\20230919\202309192000.000', column=9)
            # sta = sta[sta.data0 >= 80]
            plt.scatter(sta.lon.to_list(), sta.lat.to_list(), marker='^', s=30, c='orange')


            map.add_geometries(shp.geometries(), crs=ccrs.PlateCarree(), edgecolor='r', linewidths=0.6,
                               facecolor='none', alpha=0.6)
            filepath = os.path.join(outpathdir, 'wind_' + self.date_time.strftime('%Y%m%d%H%M') + '.png')
            plt.savefig(fname=filepath, bbox_inches='tight')
            print('绘图成功：{}'.format(os.path.basename(filepath)))
        except Exception as e:
            print(e)
    ...
    def mainProcess(self, threshold):
        ...
        RainLabelGrd = self.creat_M4_grd(lon_grd, lat_grd, label, 'windlabel', gtime)
        self.draw_rainlabel(RainLabelGrd, fy4b_channel_dict.get('BT13'))

if __name__=='__main__':
    threshold = 0.8
    outpathdir = './plot/wind'
    ...
    # drawNormClass = drawMembership(channel_list)
    # drawNormClass.draw_norm('雷暴大风-FY4B卫星通道概率密度-隶属度分布', '概率密度', '隶属度')
    # TB_dict = {
    #     'BT13': 251.5700073,
    ...
    # }
    # print('##' * 20)
    # probability, label = drawNormClass.cal_Q(TB_dict, 0.45)
