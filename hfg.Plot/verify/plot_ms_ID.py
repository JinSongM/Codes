...
def single_model_t2m_High(Infile_list, hour, is_overwrite):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    try:
...
def single_CLDAS_t2m_High(Infile_list, hour, is_overwrite):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    try:
...
def single_plot(argv, is_overwrite):
    model_name = argv[0]
    report_time = argv[1]
    var = argv[2]
    for cst in range(0, 123, 3):
        ...
        Dict_Change = {'tem_change': [file_temC, 24, 12], 'tem_change_48': [file_temC_48, 48, 12], 'sf03': [file_sd_3, 3, 3],
        ...
        Dict_tp_hour = {'pre03': [file_tp_3, 3], 'pre06': [file_tp_6, 6], 'pre12': [file_tp_12, 12], 'pre12_cp': [file_cp_12, 12]}
        if var in Dict_t2m.keys():
            single_model_t2m(Dict_t2m.get(var)[0], Dict_t2m.get(var)[1], is_overwrite)
        elif var in Dict_tem_min.keys():
            var1 = 'tem_max'
            single_model_t2m_min(Dict_tem_min.get(var)[0], Dict_t2m_High.get(var1)[0],
                                 Dict_tem_min.get(var)[1], Dict_tem_min.get(var)[2], is_overwrite)
        elif var in Dict_wind.keys():
            single_model_gustwind(Dict_wind.get(var)[0], Dict_wind.get(var)[1], is_overwrite)
        elif var in Dict_exwind.keys():
            single_model_exwind(Dict_exwind.get(var)[0], Dict_exwind.get(var)[1], is_overwrite)
        elif var in Dict_tp_sd.keys():
            single_model_tp_sd(Dict_tp_sd.get(var)[0], Dict_tp_sd.get(var)[1], is_overwrite)
        elif var in Dict_tp_sd_class.keys():
            single_model_tp_sd_class(Dict_tp_sd_class.get(var)[0], Dict_tp_sd_class.get(var)[1], is_overwrite)
        elif var in Dict_r2m.keys():
            single_model_r2m(Dict_r2m.get(var)[0], Dict_r2m.get(var)[1], is_overwrite)
        elif var in Dict_t2m_High.keys():
            var1 = 'tem_exmax'
            single_model_t2m_min(Dict_tem_min.get(var1)[0], Dict_t2m_High.get(var)[0], Dict_tem_min.get(var1)[1],
                                 Dict_tem_min.get(var1)[2], is_overwrite)
        elif var in Dict_Change.keys():
            single_model_Change(Dict_Change.get(var)[0], Dict_Change.get(var)[1], Dict_Change.get(var)[2], is_overwrite)
        elif var in Dict_tp.keys():
            single_model_tp(Dict_tp.get(var)[0], Dict_tp.get(var)[1], is_overwrite)
        elif var in Dict_tp_hour.keys():
            met_model_single_plot_tp(Dict_tp_hour.get(var)[0], Dict_tp_hour.get(var)[1], is_overwrite)
        else:
            print('输入变量有误')

# 温度、变温、相对湿度和降水——CLDAS实况数据
def single_plot_CLDAS(argv, is_overwrite):
    model_name = argv[0]
    report_time = argv[1]
    var = argv[2]
    for cst in range(2, report_time.hour + 3, 3):
        ...
        if var in Dict_other.keys():
            single_CLDAS_other(Dict_other.get(var)[0], Dict_other.get(var)[1], is_overwrite)
        elif var in Dict_temC.keys():
            single_CLDAS_temC(Dict_temC.get(var)[0], Dict_temC.get(var)[1], is_overwrite)
        elif var in Dict_t2m_High.keys():
            var1 = 'tem_exmax'
            single_CLDAS_t2m_min(Dict_tem_min.get(var1)[0], Dict_t2m_High.get(var)[0],
                                 Dict_tem_min.get(var1)[1], Dict_tem_min.get(var1)[2], is_overwrite)
        elif var in Dict_tp.keys():
            single_CLDAS_tp(Dict_tp.get(var)[0], Dict_tp.get(var)[1], is_overwrite)
        elif var in Dict_tp_hour.keys():
            met_model_single_plot_tp_CLDAS(Dict_tp_hour.get(var)[0], Dict_tp_hour.get(var)[1], is_overwrite)
        elif var in Dict_tem_min.keys():
            var1 = 'tem_max'
            single_CLDAS_t2m_min(Dict_tem_min.get(var)[0], Dict_t2m_High.get(var1)[0], Dict_tem_min.get(var)[1], Dict_tem_min.get(var)[2], is_overwrite)
        else:
            print('输入变量有误')

# 绘制模式预报风速图
def plot_wind_speed_single(argv, is_overwrite):
    model_name = argv[0]
    report_time = argv[1]
    for cst in range(0, 123, 3):
        try:
...
# 绘制CLDAS实况风速图
def plot_wind_speed_single_CLDAS(argv, is_overwrite):
    model_name = argv[0]
    report_time = argv[1]
    for cst in range(0, report_time.hour + 1, 1):
        try:
