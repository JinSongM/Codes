...
class loadData:
    ...
    def __init__(self):
        ...
        try:
            ...
            # self.rain0_df = pd.concat([self.rain_df0, self.rain_df1, self.rain_df2], axis=0)
            # self.rain1_df = pd.concat([self.rain_df2, self.rain_df3], axis=0)
            self.rain0_df = self.rain0_df.dropna(axis=0, how='any')
...
class drawMembership(loadData):
        ...
        if os.path.exists(filepath):
            ...
            try:
                channel_dict = {
                    ...
                    # 'lat': np.arange(10, 60.01, 0.1),
                    # 'lon': np.arange(70, 140.01, 0.1),
                }
    ...
    def checkAccuracy(self):
        ...
        for j in range(5, 105, 5):
            ...
            #     lines = checkdf[i:i+1]
            #     dict = lines[channel_list].to_dict('records')[0]
            #     probability, label = self.cal_Q_scatter(dict, j / 100.0)
...
class MonitorConvection:
    ...
    def MonitorRadar(self):
        ...
        try:
            ...
            fileslist = [datetime.strftime(self.date_time+timedelta(minutes=i), radar_format) for i in range(-5, 6)]
            radar_fileslist = [i for i in fileslist if os.path.exists(i)]
            radar_files = radar_fileslist[0]
    ...
    def draw_rainlabel(self, LabelGrd, background):
        try:
            ...
            # map.set_extent([112, 122, 30, 36], crs=ccrs.PlateCarree())
            ...
            # xr_data_array = morphology.remove_small_objects(xr_data_array.astype(np.bool), min_size=20, connectivity=2)
            ...
            # 绘制散点图
            sta = meb.read_stadata_from_micaps3(r'D:\Wiztek_Python\cartopy_draw\plot\data\23091920.000')
            # sta = sta[sta.data0 >= 80]
            plt.scatter(sta.lon.to_list(), sta.lat.to_list(), marker='.', s=20, c='#64FDB6')
    ...
    def mainProcess(self, threshold):
        ...
        dict_filter = {key: value for key, value in fy4b_channel_dict.items() if key in channel_list}

        label = drawNormClass.cal_Q_grd(dict_filter, threshold)
...
if __name__=='__main__':
    threshold = 0.9 #20:0.9  50: 0.94  80:0.91
    outpathdir = './plot/rain/20'
