...
class DrawImages(object):
    ...
    def __init__(self, fig=None, ax=None, shp_idx=None, axes=None, extent=None, sub_nine_dashed_extent=None, x_scale=10,
        ...
        if ax is None:
            self.fig = plt.figure(figsize=[12, 12], dpi=300)
    ...
    def set_title(self, title=None, font_size=30):
        """
    ...
    def __add_scatter(self, arr):
        ...
        if len(arr) == 5:
            self.ax.scatter(arr[0], arr[1], c=arr[2], vmin=arr[3], vmax=arr[4], cmap=plt.get_cmap("jet"), s=100)
    ...
    def add_contourf(self, lats, lons, datas, levels, label):
        cf = self.ax.contourf(lats, lons, datas, cmap=plt.get_cmap('cool'), levels=levels)
        ax2 = self.fig.add_axes([self.ax.get_position().x1 + 0.1, self.ax.get_position().y0, 0.02, self.ax.get_position().height])
        ax2.tick_params(labelsize=18)
        mpl.colorbar.ColorbarBase(ax2, cmap=plt.get_cmap('cool'), orientation='vertical', boundaries=levels)
        cbar = plt.colorbar(cf, ax2)
        cbar.set_label(label=label, fontdict={'size': 20})


    def add_color_bar(self, value_min, value_max, size, label):
        ...
        cb = mpl.colorbar.ColorbarBase(ax2, norm=norm, cmap=plt.get_cmap("jet"), orientation='vertical')
        cb.set_label(label, fontdict={'size': 20})
