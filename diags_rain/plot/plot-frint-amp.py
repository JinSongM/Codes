...
stations_lon=np.array(stations_lon).reshape((len(models),tt,4,kk))
stations_lat=np.array(stations_lat).reshape((len(models),tt,4,kk))
fcst=np.array(fcst).reshape((len(models),tt,4,kk))
obs=np.array(obs).reshape((len(models),tt,4,kk))
...
max_p_f=np.max(p_f,axis=1)
max_p_o=np.max(p_o,axis=1)
max_int_f=np.max(int_f,axis=1)
max_int_o=np.max(int_o,axis=1)

mean_p_f=np.nansum(np.nansum(i_f,axis=1),axis=1)/np.nansum(np.nansum(f_f,axis=1),axis=1)*100
mean_p_o=np.nansum(np.nansum(i_o,axis=1),axis=1)/np.nansum(np.nansum(f_o,axis=1),axis=1)*100
mean_int_f=np.nansum(np.nansum(a_f,axis=1),axis=1)/np.nansum(np.nansum(i_f,axis=1),axis=1)
mean_int_o=np.nansum(np.nansum(a_o,axis=1),axis=1)/np.nansum(np.nansum(i_o,axis=1),axis=1)

mean_p_f[np.isnan(mean_p_f)]=-999;mean_p_o[np.isnan(mean_p_o)]=-999
mean_int_f[np.isnan(mean_int_f)]=-999;mean_int_o[np.isnan(mean_int_o)]=-999

amp_p_f=100*(max_p_f-mean_p_f)/mean_p_f
amp_p_o=100*(max_p_o-mean_p_o)/mean_p_o
amp_int_f=100*(max_int_f-mean_int_f)/mean_int_f
amp_int_o=100*(max_int_o-mean_int_o)/mean_int_o
...
cs=m.scatter(longitudes, latitudes, marker='.',s=1,c=amp_p_o[0,],vmin=5,vmax=95,cmap='rainbow')
...
longitudes, latitudes = m(stations_lon[0,0,0,], stations_lat[0,0,0,])
cs=m.scatter(longitudes, latitudes, marker='.',s=1,c=amp_p_o[0,],vmin=5,vmax=95,cmap='rainbow')
pngname='AMP-freq-OBS-day'+str(ld)+'.png'
...
m.readshapefile("./shp/china_nine_dotted_line",'nine_dotted',drawbounds=True)
longitudes, latitudes = m(stations_lon[0,0,0,], stations_lat[0,0,0,])
cs=m.scatter(longitudes, latitudes, marker='.',s=1,c=amp_int_o[0,],vmin=5,vmax=95,cmap='rainbow')
cbar = m.colorbar(cs, location='bottom', pad='10%')
cbar.set_label("Percent(%)")
m.drawparallels(np.arange(slat,elat+1, 10.), labels=[1,0,0,0], fontsize=10)
m.drawmeridians(np.arange(slon,elon, 10.), labels=[0,0,0,1], fontsize=10)
...
m.drawstates()
m.drawcountries()
plt.title('Amplitude of of RAIN Int\nOBS'+'\nDate:'+str(startobs.strftime("%Y%m%d"))+'-'+str(endobs.strftime("%Y%m%d")),loc='left',fontsize=9)
left,bottom,width,height=0.75,0.27,0.17,0.2
ax3=fig.add_axes([left,bottom,width,height])
slat=0;elat=25;slon=105;elon=125
lons = np.arange(slon, elon,reso)[:]
...
longitudes, latitudes = m(stations_lon[0,0,0,], stations_lat[0,0,0,])
cs=m.scatter(longitudes, latitudes, marker='.',s=1,c=amp_int_o[0,],vmin=5,vmax=95,cmap='rainbow')
pngname='AMP-int-OBS-day'+str(ld)+'.png'
plt.savefig(pngname,dpi=120)
plt.close()
...
for mm in range(0,len(models)):
	...
	cs=m.scatter(longitudes, latitudes, marker='.',s=1,c=amp_p_f[mm,],vmin=5,vmax=95,cmap='rainbow')
	cbar = m.colorbar(cs, location='bottom', pad='10%')
	cbar.set_label("Percent(%)")
	m.drawparallels(np.arange(slat,elat+1, 10.), labels=[1,0,0,0], fontsize=10)
	m.drawmeridians(np.arange(slon,elon, 10.), labels=[0,0,0,1], fontsize=10)
	...
	longitudes, latitudes = m(stations_lon[0,0,0,], stations_lat[0,0,0,])
	cs=m.scatter(longitudes, latitudes, marker='.',s=1,c=amp_p_f[mm,],vmin=5,vmax=95,cmap='rainbow')
	pngname='AMP-freq-day-'+str(ld)+'-'+str(models[mm])+'.png'
	plt.savefig(pngname,dpi=120)
	plt.close()
	...
	m.readshapefile("./shp/china_nine_dotted_line", 'nine_dotted', drawbounds=True)
	longitudes, latitudes = m(stations_lon[0,0,0,], stations_lat[0,0,0,])
	cs=m.scatter(longitudes, latitudes, marker='.',s=1,c=amp_int_f[mm,],vmin=5,vmax=95,cmap='rainbow')
	cbar = m.colorbar(cs, location='bottom', pad='10%')
	cbar.set_label("Percent(%)")
	m.drawparallels(np.arange(slat,elat+1, 10.), labels=[1,0,0,0], fontsize=10)
	m.drawmeridians(np.arange(slon,elon, 10.), labels=[0,0,0,1], fontsize=10)
	...
	m.drawstates()
	m.drawcountries()
	plt.title('Amplitude of RAIN Int\nFCST:'+str("%9s"%models[mm])+'\nDate:'+str(starttime.strftime("%Y%m%d"))+'-'+str(endtime.strftime("%Y%m%d"))+'\nFcst:D+'+str(ld),loc='left',fontsize=9)
	left,bottom,width,height=0.75,0.27,0.17,0.2
	ax3=fig.add_axes([left,bottom,width,height])
	slat=0;elat=25;slon=105;elon=125
	lons = np.arange(slon, elon,reso)[:]
	...
	longitudes, latitudes = m(stations_lon[0,0,0,], stations_lat[0,0,0,])
	cs=m.scatter(longitudes, latitudes, marker='.',s=1,c=amp_int_f[mm,],vmin=5,vmax=95,cmap='rainbow')
	pngname='AMP-int-day-'+str(ld)+'-'+str(models[mm])+'.png'
	plt.savefig(pngname,dpi=120)
	plt.close()
