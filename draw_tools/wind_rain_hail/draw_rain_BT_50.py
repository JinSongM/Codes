...
def histogram2d_1(x_data, y_data, x_name, y_name, pic_name):
    ...
    levels = np.arange(100, np.max(heatmap_array) + int(np.max(heatmap_array)/20), int(np.max(heatmap_array)/20))
...
def histogram2d_2(x_data, y_data, x_name, y_name, pic_name):
    ...
    extent = [xedges[0], xedges[-1], yedges[0], yedges[-1]]
    levels = np.arange(100, np.max(heatmap_array) + int(np.max(heatmap_array)/30), int(np.max(heatmap_array)/30))
    x, y = np.meshgrid(xedges[:-1], yedges[:-1])
    heatmap_array = cv2.GaussianBlur(heatmap_array, (5, 5), 0, 0)
    heatmap = ax.contour(x, y, heatmap_array, levels=np.array(levels), colors='black', linewidths=2)
    if True:
...
if __name__=='__main__':
    # mergeCSV(r'/data/PRODUCT/rain_channel/50', r'/data/PRODUCT/rain_channel/50/merge/merge_rain50.csv')
    filepath = r'/data/PRODUCT/rain_channel/50/merge/merge_rain50.csv'
    data_df = pd.read_csv(filepath)
    data_df = data_df.dropna(axis=0, how='any')
    histogram2d_1(data_df.BT13, data_df.BTD09_13, 'BT13(K)', 'BTD09_13(℃)', '8月份强降水≥50_风云4B卫星通道值分布')
    histogram2d_2(data_df.BT13, data_df.BTD09_10, 'BT13(K)', 'BTD09_10(℃)', '8月份强降水≥50_风云4B卫星通道值分布')
    histogram2d_3(data_df.BT13, data_df.BTD14_13, 'BT13(K)', 'BTD14_13(℃)', '8月份强降水≥50_风云4B卫星通道值分布')
    histogram2d_4(data_df.BTD09_13, data_df.BTD14_13, 'BTD09_13(℃)', 'BTD14_13(℃)', '8月份强降水≥50_风云4B卫星通道值分布')
    histogram2d_5(data_df.BTD09_13, data_df.BTD09_10, 'BTD09_13(℃)', 'BTD09_10(℃)', '8月份强降水≥50_风云4B卫星通道值分布')
    histogram2d_6(data_df.BTD09_10, data_df.BTD14_13, 'BTD09_10(℃)', 'BTD14_13(℃)', '8月份强降水≥50_风云4B卫星通道值分布')