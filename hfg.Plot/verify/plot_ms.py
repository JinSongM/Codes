...
def single_model_tp(Infile_list, hour, region):
    ...
    try:
        ...
        if int(File_Name[-3:]) >= int(hour) and int(File_Name[-3:]) % 12 == 0:
            ...
            if not os.path.exists(Infile_list[1] + region_name + '_' + File_Name + ".png"):
...
def single_model_tp_sd(Infile_list, hour, region):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    try:
...
def single_model_tp_sd_class(Infile_list, hour, region):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    try:
...
def single_model_Change(Infile_list, hour, step, region):
    '变温/降雪/对流性/大尺度'
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
...
def single_model_r2m(Infile_list, hour, region):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    try:
...
def single_model_t2m(Infile_list, hour, region):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    try:
...
def single_model_t2m_min(Infile_list, Infile_list_t2m_High, hour, fc, region):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    ...
    try:
        ...
        if int(step_time) >= hour:
            ...
            if not os.path.exists(Infile_list[1] + region_name + '_' + File_Name + ".png"):
                ...
                drw.save()
            if fc == 'max':
                if not os.path.exists(Infile_list_t2m_High[1] + region_name + '_' + File_Name + ".png"):
                    if region_name == 'CHN' or region_name == 'JNHN':
                        add_scs_value = True
                    else:
                        add_scs_value = False
...
def single_model_gustwind(Infile_list, hour, region):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    try:
...
def single_model_exwind(Infile_list, hour, region):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    try:
...
def single_CLDAS_temC(Infile_list, hour, region):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    try:
...
def single_CLDAS_tp(Infile_list, hour, region):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    try:
...
def single_CLDAS_other(Infile_list, hour, region):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    try:
...
def single_CLDAS_t2m_min(Infile_list, Infile_list_t2m_High, hour, fc, region):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    if not os.path.exists(Infile_list_t2m_High[1]):
...
def met_model_single_plot_tp(Infile_list, step, region):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    try:
...
def met_model_single_plot_tp_CLDAS(Infile_list, step, region):
    File_Name = os.path.split(Infile_list[0])[1]
    if not os.path.exists(Infile_list[1]):
        os.makedirs(Infile_list[1])
    try:
...
def single_plot(argv):
    ...
    if var in Dict_t2m.keys():
        single_model_t2m(Dict_t2m.get(var)[0], Dict_t2m.get(var)[1], region)
    elif var in Dict_tem_min.keys():
        var1 = 'tem_max'
        single_model_t2m_min(Dict_tem_min.get(var)[0], Dict_t2m_High.get(var1)[0], Dict_tem_min.get(var)[1], Dict_tem_min.get(var)[2], region)
    elif var in Dict_wind.keys():
        single_model_gustwind(Dict_wind.get(var)[0], Dict_wind.get(var)[1], region)
    elif var in Dict_exwind.keys():
        single_model_exwind(Dict_exwind.get(var)[0], Dict_exwind.get(var)[1], region)
    elif var in Dict_tp_sd.keys():
        single_model_tp_sd(Dict_tp_sd.get(var)[0], Dict_tp_sd.get(var)[1], region)
    elif var in Dict_tp_sd_class.keys():
        single_model_tp_sd_class(Dict_tp_sd_class.get(var)[0], Dict_tp_sd_class.get(var)[1], region)
    elif var in Dict_r2m.keys():
        single_model_r2m(Dict_r2m.get(var)[0], Dict_r2m.get(var)[1], region)
    elif var in Dict_t2m_High.keys():
        var1 = 'tem_exmax'
        single_model_t2m_min(Dict_tem_min.get(var1)[0], Dict_t2m_High.get(var)[0], Dict_tem_min.get(var1)[1], Dict_tem_min.get(var1)[2], region)
    elif var in Dict_Change.keys():
        single_model_Change(Dict_Change.get(var)[0], Dict_Change.get(var)[1], Dict_Change.get(var)[2], region)
    elif var in Dict_tp.keys():
        single_model_tp(Dict_tp.get(var)[0], Dict_tp.get(var)[1], region)
    elif var in Dict_tp_hour.keys():
        met_model_single_plot_tp(Dict_tp_hour.get(var)[0], Dict_tp_hour.get(var)[1], region)
    else:
        print('输入变量有误')
...
def single_plot_CLDAS(argv):
    ...
    if var in Dict_other.keys():
        single_CLDAS_other(Dict_other.get(var)[0], Dict_other.get(var)[1], region)
    elif var in Dict_temC.keys():
        single_CLDAS_temC(Dict_temC.get(var)[0], Dict_temC.get(var)[1], region)
    elif var in Dict_t2m_High.keys():
        var1 = 'tem_exmax'
        single_CLDAS_t2m_min(Dict_tem_min.get(var1)[0], Dict_t2m_High.get(var)[0], Dict_tem_min.get(var1)[1], Dict_tem_min.get(var1)[2], region)
    elif var in Dict_tp.keys():
        single_CLDAS_tp(Dict_tp.get(var)[0], Dict_tp.get(var)[1], region)
    elif var in Dict_tp_hour.keys():
        met_model_single_plot_tp_CLDAS(Dict_tp_hour.get(var)[0], Dict_tp_hour.get(var)[1], region)
    elif var in Dict_tem_min.keys():
        var1 = 'tem_max'
        single_CLDAS_t2m_min(Dict_tem_min.get(var)[0], Dict_t2m_High.get(var1)[0], Dict_tem_min.get(var)[1], Dict_tem_min.get(var)[2], region)
    else:
        print('输入变量有误')
...
def plot_wind_speed_single(argv):
    ...
    if not os.path.exists(file_path):
        ...
        pwp.plot_hgt_wind_speed_sg(region_name, map_extent, file_path)
...
def plot_wind_speed_single_CLDAS(argv):
    ...
    if not os.path.exists(file_path):
        ...
        pwp.plot_hgt_wind_speed_sg(region, map_extent, file_path)