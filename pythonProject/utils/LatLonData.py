...
class LatLonData(object):
    ...
    def get_datas(self, lons=None, lats=None):
        src_lons = np.linspace(self.start_lon, self.end_lon, self.lon_count)
        src_lats = np.linspace(self.start_lat, self.end_lat, self.lat_count)
        points = (src_lats, src_lons)
        point = np.vstack((lats, lons)).T
        datas = interpn(points, self.data, point)
        return datas
